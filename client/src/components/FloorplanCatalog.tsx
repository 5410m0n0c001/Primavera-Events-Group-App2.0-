...
export const FloorplanCatalog: React.FC<FloorplanCatalogProps> = ({
    ...
    return (
        <div
            className={`bg-white rounded-lg shadow-lg p-4 h-full flex flex-col transition-all duration-300 ${isExpanded ? 'w-full' : 'w-full items-center p-2'}`}
        >
            <div className={`flex items-center mb-4 ${isExpanded ? 'justify-between' : 'justify-center'}`}>
                {isExpanded && <h2 className="text-xl font-bold text-gray-800 tracking-tight">Catálogo</h2>}
                <div className={`flex gap-2 ${!isExpanded && 'flex-col items-center'}`}>
                    {isExpanded && (
                        <button
